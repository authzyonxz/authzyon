...
// Inicialização segura da otplib para evitar erros de undefined no runtime
let authenticator: any;
try {
  const otplib = require("otplib");
  authenticator = otplib.authenticator || otplib;
  ...
  console.error("[2FA] Erro ao carregar otplib:", e);
}
...
export async function generateTOTPSecret(username: string, issuer: string = "AuthZyon"): Promise<{ secret: string; qrCode: string }> {
  if (!authenticator) throw new Error("Serviço de autenticação 2FA não disponível");
...
export async function enable2FA(userId: number, secret: string): Promise<string[]> {
  ...
  if (existing.length > 0) {
    await db.update(user2FA).set({ secret, isEnabled: 0, backupCodes }).where(eq(user2FA.userId, userId));
  } else {
    await db.insert(user2FA).values({ userId, secret, isEnabled: 0, backupCodes });
  }
  return backupCodes;
}
...
export async function get2FAStatus(userId: number) {
  ...
  if (record.length === 0) return { isEnabled: false, hasBackupCodes: false };
  return {
...
export async function verifyAndUseBackupCode(userId: number, code: string): Promise<boolean> {
  ...
  if (!backupCodes || !backupCodes.includes(code)) return false;
